...
if (localStorage.length == 0 || cart == null || cart.length == 0) {
    ...
    for (let i = 0; i < cart.length; i++) {
        ...
        .then(data => {
            ...
            productTotalPrice.textContent = prices += data.price * parseInt(productQuantityInput.value);
        ...
        function send() {
            ...
            // Initialisation d'un tableau products
            let products = [];
            // Insertion des indentifiants des articles dans le tableau products
            products.push(cart[i].id);

            // On appelle l'API dans le cadre d'une méthode POST 
            ...
            .then(data => {
                // Redirection vers la page Confirmation, export de l'url et du paramètre id "orderID"
                window.location.href = `./confirmation.html?id=${data.orderId}`;
                // Supression du tableau cart du localStorage
                localStorage.clear();
            })

            // la console retourne un message d'erreur si l'API n'est pas atteinte
            .catch(error => { 
